...
export default function HomeFeed() {
  ...
  return (
    <div className="signal-page flex h-screen overflow-hidden relative">
      ...
      <div className="flex w-full pt-16 h-full">
        ...
        {user ? (
          ...
          <aside className="hidden lg:flex w-80 bg-white border-r border-line flex-col h-full items-center justify-center p-6 text-center">
            <span className="text-5xl mb-4">👋</span>
            <h3 className="font-bold mb-2">Rejoignez le réseau</h3>
            <p className="text-ink-soft text-sm mb-4">Connectez-vous pour interagir.</p>
            {/* 🟢 BOUTONS QUI CHANGERONT LA VUE DE LA MODALE */}
            <button onClick={() => { setAuthView('login'); setShowAuthModal(true); }} className="signal-button w-full py-3 mb-2">Se connecter</button>
            <button onClick={() => { setAuthView('register'); setShowAuthModal(true); }} className="signal-button w-full py-3 bg-ink hover:bg-ink/90">Créer un compte</button>
          </aside>
        )}

        {/* FEED */}
        <main className="flex-1 overflow-y-auto bg-paper p-6 scroll-smooth custom-scrollbar">
          <div className="max-w-2xl mx-auto flex flex-col gap-6 pb-20">
            <h1 className="font-display text-4xl font-extrabold tracking-tight text-ink">Fil d'actualité</h1>

            {/* Créer un Post */}
            {user && (
              <div className="signal-panel w-full p-5">
                <form onSubmit={submitPost} className="flex flex-col gap-4">
                  <div className="flex gap-4">
                    ...
                    <textarea placeholder={`Quoi de neuf, ${getDisplayName(user)} ?`} value={newPostContent} onChange={(e) => setNewPostContent(e.target.value)} className="signal-input flex-1 py-3 px-4 resize-none min-h-[80px]" />
                  </div>
                  {newPostPreview && (
                    <div className="relative ml-14">
                      ...
                      <button type="button" onClick={() => { setNewPostImage(null); setNewPostPreview(null); }} className="absolute top-2 right-2 bg-ink p-1.5 rounded-full hover:bg-coral">✕</button>
                    </div>
                  )}
                  <div className="flex justify-between items-center ml-14">
                    <div className="flex gap-4 items-center">
                      ...
                      <select value={isPublicPost ? "public" : "friends"} onChange={(e) => setIsPublicPost(e.target.value === "public")} className="signal-input text-xs rounded p-1.5 outline-none">
                        <option value="public">🌐 Public</option>
                        <option value="friends">👥 Amis uniquement</option>
                      </select>
                    </div>
                    <button type="submit" disabled={isPosting || (!newPostContent.trim() && !newPostImage)} className="signal-button px-5 py-2 text-sm disabled:opacity-50">Publier</button>
                  </div>
                </form>
              </div>
            )}
            ...
            {visiblePosts.map((post) => (
              <div key={post.id} className="signal-panel w-full flex flex-col overflow-hidden">
                <div className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] uppercase font-bold text-electric-dark bg-electric/10 px-2 py-0.5 rounded-full">{post.isPublic ? 'Public' : 'Amis'}</span>
                    </div>
                ...
                <div className="px-4 py-3 flex gap-6 border-t border-line">
