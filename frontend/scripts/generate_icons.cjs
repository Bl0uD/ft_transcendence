...
const componentsFile = path.join(__dirname, '..', 'src', 'components', 'HeaderIcons.tsx');
...
for (const file of files) {
  ...
  if (file === 'dark.svg' || file === 'light.svg' || file === 'search.svg') continue;
  ...
  // Custom viewBox for globe.svg which is missing it
  let viewBox = viewBoxMatch ? viewBoxMatch[1] : (file === 'globe.svg' ? '0 0 420 420' : '0 0 24 24');
